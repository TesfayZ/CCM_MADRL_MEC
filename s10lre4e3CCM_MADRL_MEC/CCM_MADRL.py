        ...
        for b in range(self.batch_size):
            ...
            for nexta in range(self.n_agents):#to find the maxQ
                if nextactor_actions_var[b,nexta,0]>=0:
                    nextperQs.append(self.critics_target[agent_id](whole_next_states_var[b], whole_nextactor_actions_var[b], next_states_var[b,nexta,:], nextactor_actions_var[b,nexta,:]))
            if len(nextperQs)==0:
                tar_perQ = self.critics_target[agent_id](whole_next_states_var[b], whole_nextactor_actions_var[b], torch.zeros(self.state_dim), torch.zeros(self.action_dim))
            else:
                tar_perQ = max(nextperQs)
